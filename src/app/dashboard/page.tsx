...
export default function Dashboard() {
  ...
  const handleSaveClick = async (id: string) => {
    try {
      ...
      setBlogs(blogs.map(blog => (blog.id === id ? { ...blog, ...editedBlog } as Blog : blog)));
  ...
  const renderContent = () => {
    ...
    if (activeTab !== 'blogs') {
        let currentItems: (Offering | Message)[] = [];
          ...
          <li key={item.id} className="py-3">
            {editingBlogId === item.id ? (
              <div className="flex flex-col space-y-2">
                <div>
                  <label htmlFor="title" className="text-xs font-bold text-gray-400">Title</label>
                  <input type="text" name="title" id="title" value={editedBlog.title || ''} onChange={handleInputChange} className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white" />
                </div>
                <div>
                  <label htmlFor="text" className="text-xs font-bold text-gray-400">Text</label>
                  <textarea name="text" id="text" value={editedBlog.text || ''} onChange={handleInputChange} className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white" rows={4} />
                </div>
                <div>
                  <label htmlFor="image" className="text-xs font-bold text-gray-400">Image URL</label>
                  <input type="text" name="image" id="image" value={editedBlog.image || ''} onChange={handleInputChange} className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white" />
                </div>
                <div>
                  <label htmlFor="link" className="text-xs font-bold text-gray-400">Link URL</label>
                  <input type="text" name="link" id="link" value={editedBlog.link || ''} onChange={handleInputChange} className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 text-white" />
                </div>
                <div className="flex justify-end items-center space-x-2 mt-2">
                  <button onClick={() => handleSaveClick(item.id)} className="btn btn-ghost btn-sm text-green-500">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                  </button>
                  <button onClick={handleCancelEdit} className="btn btn-ghost btn-sm text-gray-500">
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center space-x-1">
                  <button onClick={() => handleEditClick(item)} className="btn btn-ghost btn-sm text-blue-400">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L15.232 5.232z" /></svg>
                  </button>
                  <button onClick={() => handleDelete('blogs', item.id)} className="btn btn-ghost btn-sm text-red-500" aria-label={`Delete blog`}>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5.5 4v6m3-6v6m1.5-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                  </button>
